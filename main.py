...
def create_pro_chart(df, name, show_days=65):
    """프로페셔널 수준 차트 생성 - 3개월치 (약 65일)"""
    
    # 최근 3개월치만 표시
    chart_df = df.tail(show_days).copy()
    ...
    # ========== 2. 매매 신호 마커 ==========
    strong_buy = chart_df[chart_df['Strong_Buy'] == True]
    normal_buy = chart_df[(~chart_df['Buy_Signal'].isna()) & (chart_df['Strong_Buy'] == False)]
    sell = chart_df[~chart_df['Sell_Signal'].isna()]
    
    # 적극매수 마커 (빨간 삼각형 + 노란 테두리)
    if len(strong_buy) > 0:
        fig.add_trace(go.Scatter(
            ...
            textfont=dict(size=11, color='#FF0000', family='Arial Black'),
            name='적극매수',
            showlegend=False
        ), row=1, col=1)
    ...
    if len(normal_buy) > 0:
        fig.add_trace(go.Scatter(
            ...
            mode='markers+text',
            marker=dict(symbol='triangle-up', size=16, color='#FF6B35'),
    ...
    if len(sell) > 0:
        fig.add_trace(go.Scatter(
            ...
            textfont=dict(size=10, color='#2979FF'),
            name='매도',
            showlegend=False
        ), row=1, col=1)
...
if analyze_btn or 'stocks_data' not in st.session_state:
    ...
    if tickers:
        ...
        for ticker in tickers:
            df, name = get_stock_data(ticker, period="6mo")
